...
import HanonComposer from '../hanon/HanonComposer';
...
type Props = {
  hanon: HanonConfig;
  keys: string[];
  onLoadHanon: (config: HanonConfig) => void;
  busy: boolean;
...
export default function LibraryPanel({
  ...
  const [category, setCategory] = useState<'hanon' | 'plans' | 'songs'>(
    loadedSongId ? 'songs' : program === 'hanon' ? 'hanon' : 'plans',
  );
  ...
  return (
    <>
      <p className={s.drawerIntro}>
        Choose what to train. Hanon, plans and imported songs share the same
      ...
      <div
        className={`${s.formatBar} ${s.libraryCategories}`}
        aria-label="Library categories"
      >
        {(['hanon', 'plans', 'songs'] as const).map((value) => (
          <button
            key={value}
      ...
      <div hidden={category !== 'hanon'} style={{ marginTop: 20 }}>
        <HanonComposer
          config={hanon}
          keys={keys}
          busy={busy}
